...

{
    ...
    public static Set<String> filterSet = new HashSet<>(Arrays.asList(SparkClac.filterWords));
    private static Set<String> allColumn = new HashSet<>(Arrays.asList("abstract_cn","title_cn","keyword_cn"));
    public static Map<String,Integer> columnIdf = new HashMap<>();
    public static double totalDoc = 0.0f;

    public static void initDf(Map<String,String> article){
        totalDoc++;
        for(String column:allColumn){
            String[] text = article.get(column).split(" ");
            Set<String> wordSet = new HashSet<>();
            wordSet.addAll(Arrays.asList(text));
            for(String word:wordSet){
                int index = WordMap.set(word);
                String key = column+"#"+index;
                if(columnIdf.containsKey(key)){
                    columnIdf.put(key,columnIdf.get(key)+1);
                }else{
                    columnIdf.put(key,1);
                }
            }
    ...
    public static void printIdf(){
        List<Map.Entry<String,Integer>> list = new ArrayList<>(columnIdf.entrySet());
        Collections.sort(list,  (a, b) -> (Double.compare(a.getValue(),b.getValue())));
        for(Map.Entry<String,Integer> entry:list){
            int v = columnIdf.get(entry.getKey());
            String col = entry.getKey().split("#")[0];
            int index = Integer.valueOf(entry.getKey().split("#")[1]);
            System.out.println(WordMap.get(index)+"#"+ Math.log10(totalDoc / (v+1)));
        }
    }
            ...
            for(String column :allColumn){
                ...
                for(int i = 0; i <  text.length; i++){
                    ...
                    for (int j = windowStart; j < windowEnd; j++) {
                        ...
                        double weight = Math.pow(E,- U * distance) ;
                        //System.out.println(WordMap.get(curWordIndex)+"#"+windowWord+"lar weight:"+weight);
                    ...
                    for(String otherCol:otherColumns){
                        ...
                        for(String key:keywords){
                            ...
                            double trans = (Math.log(otherLen / curLen+2) / Math.log(2)) *
                                    (Math.log(otherLen+1)/Math.log(3));
                            if(trans > HALF_WINDOW * 3){
                                continue;
                            }
                            //System.out.printf("cuLen:%s, otherLen:%s, trans:%s\n",curLen,otherLen,trans);

                            //System.out.println(key+"#"+1.0 /columnIdf.get(column+"#"+curWordIndex));


                            double weight = M * Math.pow(E,- U * trans)* (
                                    Math.log10(totalDoc /columnIdf.get(column+"#"+curWordIndex)));
                            System.out.println(WordMap.get(curWordIndex)+"#"+key+",trans weight:"+weight);
