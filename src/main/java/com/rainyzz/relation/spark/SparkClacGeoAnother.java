 ...
 */
public class SparkClacGeoAnother {
    ...
    public static JavaPairRDD calc(JavaRDD<String> lines) {
        ...
        JavaPairRDD<Tuple2<String, String>,Integer> pairDocCountRDD = lines.flatMapToPair(line->{
                    ...
                    String province = article.get("workplace");
                    String prefix = year +"#"+province+"#";
                    ...
                    for (String wordA : words) {
                        for (String wordB : words) {
                            ...
                            result.add(new Tuple2<>(new Tuple2<>(prefix+wordA,wordB),1));
                        }
                    }
                    return result;
                }
        ...
        JavaPairRDD<String, Integer> wordDocCountRDD = lines.flatMapToPair(line ->{
            ...
            words.forEach(w->result.add(new Tuple2<>(prefix+w, 1)));
        ...
        JavaPairRDD<Tuple2<String,String>, Double> weightRDD = lines.flatMapToPair(new PairFlatMapFunction<String, Tuple2<String, String>, Double>() {
            public Iterable<Tuple2<Tuple2<String, String>, Double>> call(String line) {
                ...
                allColumn.forEach(column -> calcWeight(prefix,article, column, result, allColumn));

                return result;
            }
            // LAR Model
            private void calcWeight(String prefix,Map<String, String> article,String column,List<Tuple2<Tuple2<String, String>, Double>> result,Set<String> allColumn){
                ...
                for (int i = 0; i < text.length; i++) {
                    ...
                    for (int j = windowStart; j < windowEnd; j++) {
                        ...
                        result.add(new Tuple2<>(new Tuple2<>(prefix+curWord, text[j]), weight));
                    }
                    //对于当前词，其与所有其他字段中词语都有关系。 diffCoCount
                    otherColumns.forEach(col -> otherColumnClac(prefix,article.get(col).split(" "), curWord, text.length, result));

                }
            }

            private void otherColumnClac(String prefix,String[] keywords, String curWord,int curLen, List<Tuple2<Tuple2<String, String>, Double>> result) {
                ...
                for (String keyword : keywords) {
                    ...
                    result.add(new Tuple2<>(new Tuple2<>(prefix+curWord, keyword), weight));
                }
            }
        }).reduceByKey((a, b) -> a + b);
        ...
        JavaPairRDD<String, Integer> wordCountRDD = lines.flatMapToPair(line -> {
            ...
            List<Tuple2<String, Integer>> result = new ArrayList<>();
            allWords.forEach(word->result.add(new Tuple2<>(prefix+word, 1)));
    ...
    public static void main(String[] args){
        SparkConf conf = new SparkConf().setAppName("Word Relation Geo Another");
