...
public class SparkClacGeo {
    ...
    public static final String GEO_COLUMN = "workplace";
    public static final String YEAR_COLUMN = "year";
    private static Set<String> allColumn = new HashSet<>(Arrays.asList("abstract_cn","title_cn","keyword_cn"));
    private static String addGeoAndTime(String word,String geo,String time){
        return word+"#"+geo+"#"+time;
    }
    public static JavaPairRDD calc(JavaRDD<String> lines) {
        ...
        JavaPairRDD<Tuple2<String, String>,Integer> pairDocCountRDD = lines.flatMapToPair(line->{
                ...
                String geo = article.get(GEO_COLUMN);
                String time = article.get(YEAR_COLUMN);
                ...
                for (String wordA : words) {
                    for (String wordB : words) {
                        ...
                        result.add(new Tuple2<>(new Tuple2<>(addGeoAndTime(wordA,geo,time),wordB),1));
                    }
                }
                return result;
            }
        ...
        JavaPairRDD<String, Integer> wordDocCountRDD = lines.flatMapToPair(line ->{
                ...
                words.forEach(w->result.add(new Tuple2<>(addGeoAndTime(w,geo,time), 1)));
            ...
            private void calcWeight(Map<String, String> article,String column,List<Tuple2<Tuple2<String, String>, Double>> result,Set<String> allColumn){
                ...
                for (int i = 0; i < text.length; i++) {
                    ...
                    for (int j = windowStart; j < windowEnd; j++) {
                        ...
                        result.add(new Tuple2<>(new Tuple2<>(addGeoAndTime(curWord,geo,time), text[j]), weight));
                    }
                    //对于当前词，其与所有其他字段中词语都有关系。 diffCoCount
                    otherColumns.forEach(col -> otherColumnClac(article.get(col).split(" "), curWord, text.length, result,geo,time));

                }
            }

            private void otherColumnClac(String[] keywords, String curWord,int curLen, List<Tuple2<Tuple2<String, String>, Double>> result,String geo,String time) {
                ...
                for (String keyword : keywords) {
                    ...
                    result.add(new Tuple2<>(new Tuple2<>(addGeoAndTime(curWord,geo,time), keyword), weight));
                }
            }
        }).reduceByKey((a, b) -> a + b);
        ...
        JavaPairRDD<String, Integer> wordCountRDD = lines.flatMapToPair(line -> {
            ...
            List<Tuple2<String, Integer>> result = new ArrayList<>();
            allWords.forEach(word->result.add(new Tuple2<>(addGeoAndTime(word,geo,time), 1)));
